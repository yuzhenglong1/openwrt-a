...
# 4. 【深度清理】彻底切断递归依赖和冲突源
# 4.1 物理删除导致递归依赖的包 (使用模糊匹配确保清理干净)
# 这些包在 kenzo/small 仓库中经常导致 Kconfig 递归错误，必须物理删除其源码目录
echo "正在清理冲突插件..."
CONFLICT_PKGS=("homeproxy" "momo" "fchomo" "nikki" "daed" "v2ray-geodata" "sing-box" "mihomo" "open-app-filter")
for pkg in "${CONFLICT_PKGS[@]}"; do
    find package/ feeds/ -type d -name "*$pkg*" -exec rm -rf {} +
done

# 4.2 物理删除官方源码中报错的 ksmbd (与新内核 6.12 冲突)
rm -rf package/kernel/ksmbd
find package/ feeds/ -type d -name "*ksmbd*" -exec rm -rf {} +

# 4.3 清理其他已知冲突、重复或缺失依赖的包
# 重点清理 luci-app-ssr-plus，因为它在 main 分支上极易报错
BAD_PKGS=("luci-app-ssr-plus" "luci-app-mjpg-streamer" "onionshare-cli" "trojan" "luci-theme-alpha" "luci-app-v2ray-server" "luci-app-bypass" "luci-app-smartdns")
for bpkg in "${BAD_PKGS[@]}"; do
    find package/ feeds/ -type d -name "*$bpkg*" -exec rm -rf {} +
done

# 4.4 修复核心插件兼容性 (注入 luci-compat)
# OpenWrt Main 分支去掉了许多旧版 LuCI 库，必须手动补齐依赖
...
find package/feeds/ -name "luci-app-mosdns" -type d -exec sh -c 'sed -i "s/DEPENDS:=+luci-base/DEPENDS:=+luci-base +luci-compat/g" {}/Makefile' \;
